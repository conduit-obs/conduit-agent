...
| File | What it ships |
|---|---|
| [`macos-host-overview.json`](macos-host-overview.json) | Default per-host overview for the darwin profile: load avg, CPU/memory/filesystem **utilization** (percent-form), disk + network throughput, TCP connections, log volume by severity, top log templates by `process` + `log.record.template`. |
...
| [`windows-host-overview.json`](windows-host-overview.json) | Default board for the windows profile (M6.D). Two halves: per-host **resource pressure** broken down by `host.name` (CPU Queue Length as the saturation indicator instead of Unix load average — the Windows `load` scraper reads `System\Processor Queue Length` and emits it as `system.cpu.load_average.1m`) and a **Windows Event Log** narrative broken down by `winlog.channel` + `winlog.provider_name` + `winlog.event_id`. Severity uses OTel's `severity_text` (Critical → FATAL, Error → ERROR, Warning → WARN, Information → INFO, Verbose → DEBUG) so cross-platform severity panels work without a Windows-specific case. |

Future platform boards land alongside their respective platform milestones — `k8s-workload-overview.json` as a follow-up to M5.E once we have field signal on whether deployment-scoped vs pod-scoped is the more useful default. Each is its own opinionated, narrative-driven dashboard tailored to what's distinctive about that platform's telemetry, **not** a copy of the host-overview skeleton — a Datadog-quality experience comes from each platform feeling first-class, not from forced uniformity.
