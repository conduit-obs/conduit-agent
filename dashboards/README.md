...
Future platform boards land alongside their respective platform milestones — `windows-host-overview.json` at M6, `k8s-cluster-overview.json` at M5, `docker-host-overview.json` at M9 (it depends on M9's host-metrics-from-container work to have populated panels). Each is its own opinionated, narrative-driven dashboard tailored to what's distinctive about that platform's telemetry, **not** a copy of the host-overview skeleton — a Datadog-quality experience comes from each platform feeling first-class, not from forced uniformity.
