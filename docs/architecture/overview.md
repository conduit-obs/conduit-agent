...
| `transform/logs` processor | Logs pipeline only. Runs OTTL rules that redact common credential shapes (`Authorization` headers, AWS access key IDs, etc.) and structure common log formats: JSON and logfmt bodies get trace/span ids, message, and severity lifted onto the OTel fields; klog headers and leading level tokens ("ERROR: ...") map to severity. See [ADR-0010](../adr/adr-0010.md) and [ADR-0024](../adr/adr-0024.md). |
| `drain` processor | Logs pipeline only, after `transform/logs`. Clusters each (redacted) log body with the Drain algorithm and annotates the record with its template string as `log.record.template` — the stable "group similar lines" column the dashboards' Top Log Templates panels key off. See [ADR-0024](../adr/adr-0024.md). |
